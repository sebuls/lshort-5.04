...
\TeX 은 \index{Knuth, Donald E.}\index{크누쓰}도널드 크누쓰(Donald E.\
...
% becomes \texttt{TeX}.
\TeX 은 ``tech''로 발음한다. 여기서 ``ch''는 독일어 ``Ach''에서의
...
%anywhere.
  1994년에 프랑크 미텔바흐가 주도하는 \LaTeX 3
  팀\index{LaTeX3@\LaTeX 3}이 \LaTeX\ 패키지를 개정하였다. 여기서 그 동안
  누적되어 온 개선요구가 받아들여지고 그보다 몇 해 전 \index{LaTeX
    2.09@\LaTeX{} 2.09}\LaTeX{} 2.09가 발표된 이래 산발적으로 쌓여 온
  패치 버전을 전부 통합하여 재정비하였다. 새로운 버전을 이전 것과
  구별하기 위하여 \index{LaTeX2e@\LaTeXe}\LaTeXe 라고 부른다. 이 책자가
...
\texttt{LaTeX}으로 쓴다. \LaTeXe는 ``Lay-tech two e''라고
...
\TeX{}을 조판기로 이용한다. 그러나 \LaTeX은 컴퓨터 프로그램에
...
이러한 방식은 이른바 \wi{WYSIWYG}\footnote{What you see is
  what you get.}이라 불리는,
...
\LaTeX{}을 사용하면 텍스트를 입력하는 동안 최종 출력물의 모양을 볼 수 없지만,
